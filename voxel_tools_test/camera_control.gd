...
var _local_direction = Vector3(0.0, 0.0, 0.0)
var _global_direction = Vector3(0.0, 0.0, 0.0)
var _local_speed = Vector3(0.0, 0.0, 0.0)
var _global_speed = Vector3(0.0, 0.0, 0.0)
var _gui
...
func _input(event):
	...
	if movement:
		if event.is_action_pressed(forward_action):
			...
			_local_direction.z = 0

		if event.is_action_pressed(left_action):
			_local_direction.x = -1
			...
			_local_direction.x = 0
			
		if event.is_action_pressed(up_action):
			_global_direction.y = 1
...
func _update_movement(delta):
	var local_offset = max_speed * acceleration * _local_direction
	var global_offset = max_speed * acceleration * _global_direction
	
	_local_speed.x = clamp(_local_speed.x + local_offset.x, -max_speed.x, max_speed.x)
	_local_speed.y = clamp(_local_speed.y + local_offset.y, -max_speed.y, max_speed.y)
	_local_speed.z = clamp(_local_speed.z + local_offset.z, -max_speed.z, max_speed.z)

	_global_speed.x = clamp(_global_speed.x + global_offset.x, -max_speed.x, max_speed.x)
	_global_speed.y = clamp(_global_speed.y + global_offset.y, -max_speed.y, max_speed.y)
	_global_speed.z = clamp(_global_speed.z + global_offset.z, -max_speed.z, max_speed.z)

	# Apply deceleration if no input
	if _local_direction.x == 0:
		_local_speed.x *= (1.0 - deceleration)
	if _local_direction.y == 0:
		_local_speed.y *= (1.0 - deceleration)
	if _local_direction.z == 0:
		_local_speed.z *= (1.0 - deceleration)

	# Apply deceleration if no input
	if _global_direction.x == 0:
		_global_speed.x *= (1.0 - deceleration)
	if _global_direction.y == 0:
		_global_speed.y *= (1.0 - deceleration)
	if _global_direction.z == 0:
		_global_speed.z *= (1.0 - deceleration)

	translate(_local_speed * delta)
	global_translate(_global_speed * delta)
